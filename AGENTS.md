...
- **Installed ECC Location**: Installed ECC plugin assets reside strictly in their target locations: agents in `.agents/agents/<name>/agent.md`, rules in `.agents/rules/<name>.md` (flat layout), workflows and bridge workflows in `.agents/workflows/<name>.md` (flat layout), skills in `.agents/skills/<skill-name>/SKILL.md`, and lifecycle hooks configuration at `.agents/hooks.json`. Never write installed assets directly into legacy `.agent/`.
- **Subagent Structure Standard**: ECC agents MUST be converted and stored as `.agents/agents/<name>/agent.md` along with supporting prompts, schemas, or references inside `.agents/agents/<name>/`. All `agent.md` files MUST contain valid 8-field YAML frontmatter (`name`, `description`, `mainAgent`, `subagent`, `model`, `tools`, `mcpServers`, `skills`) validated via `verify-installation-agy.js`.
...
- **Proposal Compliance Verification (OBJ-01)**: Verification scripts such as `harness/agy-script/scripts/verify-installation-agy.js` MUST enforce Fail-Fast validation (exiting with code 1 upon any discrepancy) when checking physical disk installation against Section 2.2 of `docs/OBJ-01/artifacts/proposal.md` across all 6 item kinds (`rules` in `.agents/rules/`, `agents` in `.agents/agents/`, `commands`/`workflows` in `.agents/workflows/`, `hooks` at `.agents/hooks.json`, `skills` in `.agents/skills/`, `platform` in `.agents/plugin/ecc/platform/`).
...
## 11. 100% Self-Contained /.agents/scripts/ Script Management & Library Co-location Invariant
- **100% Self-Contained Script Harness Location**: All post-installation runtime scripts (including lifecycle hook scripts, support scripts, workflow and command helpers, skill runners, multi-agent orchestrators, platform tools, guardrails, and observation envelope adapters) MUST reside directly inside [.agents/scripts/](file:///d:/dev/agy-os/.agents/scripts/). All shared helper modules and runtime libraries MUST be co-located directly inside [.agents/scripts/lib/](file:///d:/dev/agy-os/.agents/scripts/lib/). Custom AGY-native runtime scripts MUST continue using the `-agy.js` suffix convention (e.g., [.agents/scripts/pre-tool-guardrail-agy.js](file:///d:/dev/agy-os/.agents/scripts/pre-tool-guardrail-agy.js), [.agents/scripts/observation-envelope-agy.js](file:///d:/dev/agy-os/.agents/scripts/observation-envelope-agy.js)).
- **Separation of Installer vs. Runtime**: Installer, setup, verification, and teardown scripts MUST remain strictly under [harness/agy-script/](file:///d:/dev/agy-os/harness/agy-script/) (`install-agy.sh`, `install-apply-agy.js`, `verify-installation-agy.js`, `uninstall-agy.sh`), while active post-installation runtime scripts and shared libraries MUST reside strictly under [.agents/scripts/](file:///d:/dev/agy-os/.agents/scripts/) and [.agents/scripts/lib/](file:///d:/dev/agy-os/.agents/scripts/lib/).
- **Zero External Environment Variable Dependency**: All runtime script execution and internal module imports MUST resolve relative to [.agents/scripts/](file:///d:/dev/agy-os/.agents/scripts/) with zero requirement for external environment variables (such as `CLAUDE_PLUGIN_ROOT`), external directory paths, dynamic path adapters, or shims. Harness script execution within [agy-os](file:///d:/dev/agy-os) MUST be 100% self-contained, autonomous, and portable.
