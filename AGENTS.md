...
- **Runtime Script Management Harness Location**: All post-installation runtime script management harness modules (including lifecycle hook interceptors, PreToolUse/PostToolUse guardrails, tool observation envelope adapters, and runtime error contract handlers) MUST reside directly inside `.agents/hooks/scripts/` and strictly use the `-agy.js` suffix convention (e.g., `.agents/hooks/scripts/pre-tool-guardrail-agy.js`, `.agents/hooks/scripts/observation-envelope-agy.js`).
